...
function Base.show(io::IO, ::MIME"text/plain", bi::PllBodeInfo)
	pmfstr = num2si(String,fϕ,1)
	pmystr = num2si(String,ϕₘ,1)
	gmfstr = num2si(String,fg,1)
	gmystr = num2si(String,gₘ,1)
...
function pllbodeinfo(pll::PLL)
	...
	m₀(f::Float64) = 20*log10(abs(OL(2im*pi*f)))

	# Function for phase+π:
	ϕ₀(f::Float64) = angle(OL(2im*pi*f))+π
